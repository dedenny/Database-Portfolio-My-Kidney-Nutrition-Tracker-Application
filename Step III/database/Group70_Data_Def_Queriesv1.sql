...
CREATE TABLE `Dialysis_Forms` (
  `dialysis_id` int(10) unsigned AUTO_INCREMENT,
...
CREATE TABLE `Lab_Results` (
  ...
  CONSTRAINT `fk_Lab_Results_Dialysis_Forms1` FOREIGN KEY (`Dialysis_Forms_dialysis_id`) REFERENCES `Dialysis_Forms` (`dialysis_id`) ON DELETE SET NULL ON UPDATE NO ACTION,
  CONSTRAINT `fk_Lab_Results_Patients1` FOREIGN KEY (`Patients_patient_id`) REFERENCES `Patients` (`patient_id`) ON DELETE SET NULL ON UPDATE NO ACTION
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;
